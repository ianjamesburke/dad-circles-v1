...
import { maskEmail, maskIP } from './utils/pii';
...
export class RateLimiter {
  ...
  /**
   * Check if a magic link request should be allowed from this IP address
   *
   * @param ip - IP address to check
   * @returns Object with allowed flag and optional reason
   */
  static async checkMagicLinkRequestByIP(ip: string): Promise<{ allowed: boolean; reason?: string }> {
    return this.check(
      ip,
      CONFIG.rateLimits.magicLinkByIP,
      maskIP
    );
  }
  ...
  /**
   * Check if weekend mission generation should be allowed for this identifier
   *
   * @param identifier - Typically ip:postcode for public mission endpoint
   * @returns Object with allowed flag and optional reason
   */
  static async checkWeekendMissionRequest(identifier: string): Promise<{ allowed: boolean; reason?: string }> {
    return this.check(
      identifier,
      CONFIG.rateLimits.weekendMission
    );
  }
