  ...
  switch (step) {
    ...
    case OnboardingStep.LOCATION: {
      ...
      if (location) {
        ...
        return `I have you in ${locationText}. Is that correct?`;
      }
  ...
  switch (step) {
    ...
    case OnboardingStep.LOCATION: {
      ...
      if (location) {
        ...
        return `Confirm the inferred location: ${locationText}. Ask if it is correct.`;
      }
      return "Ask for their city and state/region.";
    }
    case OnboardingStep.CONFIRM:
...
export const getGeminiResponse = onCall(
  ...
  async (request) => {
    ...
    const systemInstruction = [
      ...
      "Collect info in order: name, dad status, kids info, interests, location confirmation, summary confirmation.",
      ...
      "If location is inferred from postcode, ask the user to confirm it.",
      "Only mark onboarding complete after an explicit confirmation of the summary.",
      "Required for completion: name, dad status, at least one child with birth year, interests (can be empty), and location (city + state).",
